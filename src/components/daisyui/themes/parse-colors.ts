...
import { themeDefaults,colorNames } from "./defaults";
import { oklch, interpolate, wcagContrast, Color } from "culori";
export type ColorsParameter = Parameters<typeof interpolate>[0][0]
...
export const parseColors = {
  ...
  colorObjToString: (input: { l: number; c: number; h: number }) => {
    const { l, c, h } = input;
    return `${Number.parseFloat((cutNumber(l) * 100).toFixed(6))}% ${cutNumber(c)} ${cutNumber(h)}`;
  },

  generateForegroundColorFrom: function (input: Color | string, percentage = 0.8) {
    try {
      const result = interpolate(
        [input, this.isDark(input) ? "white" : "black"],
        "oklch"
      )(percentage);
      return this.colorObjToString({ c: result.c, l: result.l, h: result.h ?? 0 });
    } catch (e) {
  ...
  convertColorFormat: function (input:Record<string,any>) {
    ...
    for (const [rule, value] of Object.entries(input)) {
      if (Object.hasOwn(colorNames, rule)) {
        try {
          ...
          resultObj[colorNames[rule]] = this.colorObjToString({c:colorObj.c,l:colorObj.l,h:colorObj.h});
        } catch (e) {
